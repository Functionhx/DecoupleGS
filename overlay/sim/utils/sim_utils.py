...
def create_cam(intrinsic, c2w):
    # Keep geometry/controller helpers importable without the optional scene
    # I/O stack (plyfile, rasterizer extensions).
...
def trajectory_to_ilqr_reference(plan_traj):
    """Convert planner ``[right, forward]`` points to iLQR state targets.

    The iLQR bicycle model uses conventional ``[forward, right, yaw, ...]``
    state axes.  Heading must therefore be computed *after* the position-axis
    swap as ``atan2(delta_right, delta_forward)``.  The released helper used
    the pre-swap order, mapping a straight trajectory to roughly +/-90 deg.
    """

    plan_traj = np.asarray(plan_traj, dtype=np.float64)
    if plan_traj.ndim != 2 or plan_traj.shape[0] == 0 or plan_traj.shape[1] < 2:
        raise ValueError(f"plan_traj must have shape [N, >=2], got {plan_traj.shape}")
    reference = np.zeros((plan_traj.shape[0] + 1, 5), dtype=np.float64)
    reference[1:, :2] = plan_traj[:, [1, 0]]
    delta = np.diff(reference[:, :2], axis=0)
    headings = np.zeros(len(delta), dtype=np.float64)
    last_heading = 0.0
    for index, (delta_forward, delta_right) in enumerate(delta):
        if math.hypot(delta_forward, delta_right) > 1e-9:
            last_heading = math.atan2(delta_right, delta_forward)
        headings[index] = last_heading
    reference[1:, 2] = np.unwrap(headings)
    return reference
...
def traj2control(plan_traj, info):
    ...
    """
    plan_traj_stats = trajectory_to_ilqr_reference(plan_traj)
    curr_stat = np.array(
